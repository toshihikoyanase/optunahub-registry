...
EPS = 1e-10
_NOISY_PARAMS_KEY = "noisy_params"
...
class RobustGPSampler(BaseSampler):
    ...
    Args:
        ...
        noisy_suggestion:
            If this option is enabled, suggested values will be the nominal value with the
            nominal value plus added noise without clipping. This option is useful when a user want
            to evaluate the objective function against values with added noise rather than
            nominal values. If this option is enabled, noisy values can be retrieved using
            `get_noisy_params`.
    """
    ...
    def sample_relative(
        ...
        params = self._optimize_params(study, trials, search_space, const_noisy_param_values)

        if self._noisy_suggestion:
            ...
            if self._uniform_input_noise_rads is not None:
                for param_name, rad in self._uniform_input_noise_rads.items():
                    ...
                    assert not dist.log
                    noisy_params[param_name] = params[param_name] + self._rng.rng.uniform(
                        -rad, rad
                    )
            elif self._normal_input_noise_stdevs is not None:
                for param_name, std in self._normal_input_noise_stdevs.items():
                    ...
                    assert dist.step is None
                    assert not dist.log
                    noisy_params[param_name] = params[param_name] + self._rng.rng.normal(scale=std)
            else:
                assert False, "Should not reach here."
            study._storage.set_trial_system_attr(trial._trial_id, _NOISY_PARAMS_KEY, noisy_params)
        return params
    ...
    def get_noisy_params(self, trial: FrozenTrial | optuna.Trial) -> dict[str, Any]:
        if _NOISY_PARAMS_KEY in trial.system_attrs:
            return trial.system_attrs[_NOISY_PARAMS_KEY]
        else:
            return trial.params
